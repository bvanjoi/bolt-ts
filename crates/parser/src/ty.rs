...
impl<'cx> ParserState<'cx, '_> {
    ...
    fn parse_union_or_intersection_ty<const IS_UNION_TY: bool>(
        ...
        if self.token.kind == expect {
            ...
            let span = self.new_span(ty.span().lo());
            let ty = if !IS_UNION_TY {
                let intersection = self.create_intersection_type(span, tys);
                self.alloc(ast::Ty {
                    kind: ast::TyKind::Intersection(intersection),
                })
            } else {
                // union
                debug_assert_eq!(expect, TokenKind::Pipe);
                let union = self.create_union_type(span, tys);
                self.alloc(ast::Ty {
                    kind: ast::TyKind::Union(union),
                })
            };
            Ok(ty)
        } else {
    ...
    fn parse_infer_ty(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        ...
        let infer = self.create_infer_type(self.new_span(start), ty_param);
        let ty = self.alloc(ast::Ty {
            kind: ast::TyKind::Infer(infer),
        });
        Ok(ty)
    }
    ...
    fn parse_ty_param_of_infer_ty(&mut self) -> PResult<&'cx ast::TyParam<'cx>> {
        ...
        let span = self.new_span(start);
        let ty = self.create_type_parameter(span, None, name, constraint, None);
        Ok(ty)
    }
            ...
            match self.token.kind {
                ...
                TokenKind::LBracket => {
                    ...
                    if self.is_start_of_ty(false) {
                        ...
                        self.expect(TokenKind::RBracket);
                        let kind = self.create_array_type(self.new_span(ty.span().lo()), ty);
                        ty = self.alloc(ast::Ty {
    ...
    pub(super) fn parse_entity_name<const ALLOW_RESERVED_WORD: bool>(
        ...
        while self.parse_optional(TokenKind::Dot).is_some() {
            ...
            let qualified = self.create_qualified_name(self.new_span(start), entity, right);
    ...
    fn parse_ty_query(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        ...
        let kind = self.create_typeof_type(self.new_span(start), name, ty_args);
        let ty = self.alloc(ast::Ty {
    ...
    fn parse_this_ty_kind(&mut self) -> &'cx ast::ThisTy {
        ...
        self.create_this_type(self.new_span(start))
    }
    ...
    fn parse_asserts_ty_pred(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        ...
        let span = self.new_span(pos);
        let ty = self.create_type_predicate(span, Some(assert_modifier), name, ty);
        let ty = self.alloc(ast::Ty {
            kind: ast::TyKind::Pred(ty),
        });
        Ok(ty)
    ...
    fn parse_mapped_ty(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        ...
        let kind = self.create_mapped_type(
            self.new_span(start),
            readonly_token,
            ty_param,
            name_ty,
    ...
    fn parse_tuple_ty(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        ...
        let tuple = self.create_tuple_type(self.new_span(start), tys);
        let ty = self.alloc(ast::Ty {
            kind: ast::TyKind::Tuple(tuple),
        });
        Ok(ty)
    }
    ...
    fn parse_tuple_ele_name_or_tuple_ele_ty(&mut self) -> PResult<&'cx ast::Ty<'cx>> {
        if self.lookahead(Lookahead::is_tuple_ele_name) {
            ...
            let named = self.create_named_tuple_type(
                self.new_span(start),
                dotdotdot.map(|t| t.span),
                name,
                question.map(|t| t.span),
                ty,
            );
            let ty = self.alloc(ast::Ty {
                kind: ast::TyKind::NamedTuple(named),
            });
            Ok(ty)
        } else {
    ...
    fn parse_prop_or_method_sig(
        ...
        let kind = if matches!(self.token.kind, TokenKind::LParen | TokenKind::Less) {
            ...
            let ty = self.parse_ty_anno()?;
            let sig =
                self.create_property_signature(self.new_span(start), modifiers, name, question, ty);
    ...
    fn parse_sig_member<const IS_CALL: bool>(&mut self) -> PResult<&'cx ast::ObjectTyMember<'cx>> {
        ...
        let span = self.new_span(start);
        let kind = if IS_CALL {
            let decl = self.create_call_signature_declaration(span, ty_params, params, ty);
            ast::ObjectTyMemberKind::CallSig(decl)
        } else {
            let decl = self.create_constructor_signature_declaration(span, ty_params, params, ty);
    ...
    pub(super) fn parse_object_ty_members(&mut self) -> ast::ObjectTyMembers<'cx> {
