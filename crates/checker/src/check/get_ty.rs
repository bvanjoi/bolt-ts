...
use bolt_ts_ast::{pprint_binding, r#trait};
...
impl<'cx> TyChecker<'cx> {
    pub(super) fn get_non_missing_type_of_symbol(&mut self, id: SymbolID) -> &'cx Ty<'cx> {
        let t = self.get_type_of_symbol(id);
        let is_optional = self.symbol(id).flags.contains(SymbolFlags::OPTIONAL);
        self.remove_missing_ty(t, is_optional)
    }
    ...
    fn get_ty_of_var_or_param_or_prop_worker(&mut self, symbol: SymbolID) -> &'cx ty::Ty<'cx> {
            ...
            || node.is_program()
        {
            ...
            self.get_widened_ty_for_var_like_decl::<true>(n)
    ...
    fn get_ty_of_func_class_enum_module(&mut self, symbol: SymbolID) -> &'cx Ty<'cx> {
        ...
        let s = self.symbol(symbol);
        // get_type_of_func_class_enum_module_worker
    ...
    fn get_ty_from_infer_ty_node(&mut self, node: &'cx ast::InferTy<'cx>) -> &'cx Ty<'cx> {
        ...
        let symbol = self.get_symbol_of_declaration(node.ty_param.id);
    ...
    pub(super) fn get_actual_ty_variable(&mut self, ty: &'cx Ty<'cx>) -> &'cx Ty<'cx> {
        match ty.kind {
            ty::TyKind::Substitution(t) => self.get_actual_ty_variable(t.base_ty),
            ty::TyKind::IndexedAccess(t)
                if t.object_ty.flags.contains(TypeFlags::SUBSTITUTION)
                    || t.index_ty.flags.contains(TypeFlags::SUBSTITUTION) =>
            {
                let object_ty = self.get_actual_ty_variable(t.object_ty);
                let index_ty = self.get_actual_ty_variable(t.index_ty);
                self.get_indexed_access_ty(object_ty, index_ty, None, None, None, None)
            }
            _ => ty,
        }
    }
    ...
    pub(super) fn get_es_symbol_like_ty_for_node(&mut self, p: ast::NodeID) -> &'cx Ty<'cx> {
    ...
    fn get_ty_from_intersection_ty_node(
        ...
        let no_super_ty_reduction = t.flags.intersects(
            TypeFlags::STRING
    ...
    pub(super) fn get_ty_from_object_lit_or_fn_or_ctor_ty_node(
        ...
        let res = if node_symbol.is_none_or(|node_symbol| {
            ...
            let alias_ty_arguments = self.get_ty_args_for_alias_symbol(alias_symbol);
            self.create_anonymous_ty(
                node_symbol,
                ty::ObjectFlags::ANONYMOUS,
                None,
                alias_symbol,
                alias_ty_arguments,
                None,
    ...
    fn get_property_ty_for_index_type(
        &mut self,
                ...
                if index >= 0. {
                    ...
                    return Some(
                        self.get_tuple_element_ty_out_of_start_count(
                            ...
                            index as usize,
                            access_flags
                                .contains(AccessFlags::INCLUDE_UNDEFINED)
        ...
        {
            const ANY_OR_NEVER: TypeFlags = TypeFlags::ANY.union(TypeFlags::NEVER);
            if object_ty.flags.intersects(ANY_OR_NEVER) {
            ...
            if let Some(index_info) = index_info {
                if access_flags.contains(AccessFlags::NO_INDEX_SIGNATURES)
                    && index_info.key_ty != self.number_ty
                {
                    if let Some(access_expr) = access_expr {
                    ...
                    )
                {
                    ...
                    self.push_error(Box::new(error));
                    return Some(if access_flags.contains(AccessFlags::INCLUDE_UNDEFINED) {
                        self.get_union_ty::<false>(
                            &[index_info.val_ty, self.missing_ty],
                            ty::UnionReduction::Lit,
                            None,
                            None,
                            None,
    ...
    pub(super) fn get_indexed_access_ty(
        ...
        if let Some(ty) = self.get_indexed_access_type_or_undefined(
            object_ty,
            index_ty,
            access_flags,
            access_node,
    ...
    pub(super) fn get_indexed_access_type_or_undefined(
        ...
        if is_generic_index {
            ...
            // TODO: alias symbol and alias ty arguments
            ...
            && let Some(union) = index_ty.kind.as_union()
        {
            ...
            for t in union.tys.iter() {
                if let Some(prop_ty) = self.get_property_ty_for_index_type(
                    object_ty,
                    apparent_object_ty,
                    t,
                    access_node,
        ...
        self.get_property_ty_for_index_type(
            object_ty,
            apparent_object_ty,
            index_ty,
            access_node,
    ...
    pub(super) fn get_ty_from_indexed_access_node(
        ...
        let potential_alias = self.get_alias_symbol_for_ty_node(node.id);
        let alias_ty_arguments = self.get_ty_args_for_alias_symbol(potential_alias);
        let ty = self.get_indexed_access_ty(
            object_ty,
            index_ty,
            None,
            Some(node.id),
    ...
    pub(super) fn is_deferred_ty_reference_node(
        ...
        self.get_alias_symbol_for_ty_node(node).is_some()
            || self
                ...
                .is_resolved_by_type_alias(node)
                && {
                    use bolt_ts_ast::Node::*;
                    let n = self.p.node(node);
                    match n {
    ...
    pub(super) fn get_alias_symbol_for_ty_node(&self, node: ast::NodeID) -> Option<SymbolID> {
        ...
        while let Some(parent_id) = parent {
            ...
            match parent_node {
                ast::Node::ParenTy(_) => parent = self.parent(parent_id),
                ast::Node::TyOp(n) if n.op == ast::TyOpKind::Readonly => {
    ...
    pub(super) fn get_local_ty_params_of_class_or_interface_or_type_alias(
        ...
        res.map(|res| self.alloc(res) as _)
    }
        ...
        let result = loop {
            ...
            let check_tuples =
                check_ty_node.is_simple_tuple_ty() && extends_ty_node.is_simple_tuple_ty() && {
                    ...
                    let ast::TyKind::Tuple(b) = extends_ty_node.kind else {
                        unreachable!()
                    };
                    a.tys.len() == b.tys.len()
                };
            ...
            let extends_ty = self.instantiate_ty(root.extends_ty, mapper);
            let conditional_links = self.conditional_links_arena.alloc(Default::default());
    ...
    pub(super) fn get_return_type_from_body(
        ...
        match body {
            ast::ArrowFnExprBody::Expr(expr) => {
                let check_mode =
                    check_mode.map(|m| m.intersection(CheckMode::SKIP_GENERIC_FUNCTIONS.not()));
                let mut ty = self.check_expression_cached(expr, check_mode);
            ...
            ast::ArrowFnExprBody::Block(body) => {
                if is_generator {
                    match self.check_and_aggregate_return_expression_tys(id, body, check_mode) {
                    ...
                    let (yield_tys, next_tys) =
                        self.check_and_aggregate_yield_operand_tys(id, body, is_async, check_mode);
                    ...
                    let Some(tys) =
                        self.check_and_aggregate_return_expression_tys(id, body, check_mode)
                    else {
                        return if is_async {
                ...
                || next_ty.is_some_and(|next_ty| next_ty.is_unit())
            {
                ...
                let contextual_ty = contextual_sig.and_then(|sig| {
                    if sig == self.get_sig_from_decl(id) {
                        ...
                        let ret_t = self.get_return_type_of_signature(sig);
    ...
    fn is_key_ty_included(&mut self, key_ty: &'cx ty::Ty<'cx>, include: TypeFlags) -> bool {
        key_ty.flags.intersects(include)
            || key_ty
                .kind
                .as_intersection()
                .is_some_and(|i| i.tys.iter().any(|t| self.is_key_ty_included(t, include)))
    }
    ...
    fn get_literal_ty_from_props(
        ...
        let origin = if include_origin
            && (ty
                ...
                || ty.alias_symbol().is_some())
        {
            Some(self.create_index_ty(ty, IndexFlags::empty()))
        } else {
            None
        };
        ...
        let index_key_tys = self.get_index_infos_of_ty(ty).iter().map(|info| {
            if !std::ptr::eq(info, &self.enum_number_index_info())
                && self.is_key_ty_included(info.key_ty, include)
            {
