// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/mappedTypeIndexedAccessConstraint.ts`, Apache-2.0 License
...
function foo<K extends keyof M0>(m1: M1[K], m2: M2[K], m3: M3[K]) {
    m1.toString();  // Error
    //~^ ERROR: 'm1' is possibly 'undefined'.
    m1?.toString();
    m2.toString();  // Error
    //~^ ERROR: 'm2' is possibly 'undefined'.
    m2?.toString();
    m3.toString();  // Error
    //~^ ERROR: 'm3' is possibly 'undefined'.
    m3?.toString();
}
...
//~^ ERROR: 'mapped[key]' is possibly 'undefined'.
