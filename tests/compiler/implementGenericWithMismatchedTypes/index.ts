// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/implementGenericWithMismatchedTypes.ts`, Apache-2.0 License
...
class C2<T> implements IFoo2<T> { // error
  ...
  //~^ ERROR: Property 'foo' in type 'C2<T, C2>' is not assignable to the same property in base type 'IFoo2<T, C2>'.
        return null;
        //~^ ERROR: Type 'null' is not assignable to type 'number'.
    }
}