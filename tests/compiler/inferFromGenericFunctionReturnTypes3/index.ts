// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/inferFromGenericFunctionReturnTypes3.ts`, Apache-2.0 License
...
function wrappedBar(): Wrap<'bar'> {
    ...
    //~^ ERROR: Argument of type 'string' is not assignable to parameter of type '"bar"'.
    return wrap(value);
}
...
//~^ ERROR: Type '{ state: State.A; }[] | { state: State.B; }[]' is not assignable to type '{ state: State.A; }[]'.
