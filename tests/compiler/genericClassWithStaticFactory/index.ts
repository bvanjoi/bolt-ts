// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/genericClassWithStaticFactory.ts`, Apache-2.0 License
...
namespace Editor {
    ...
    export class List<T> {
        ...
        public popEntry(head: List<T>): List<T> {
            if (this.next.isHead) {
                return null;
                //~^ ERROR: Type 'null' is not assignable to type 'Editor.List<T>'.
            }
            else {
