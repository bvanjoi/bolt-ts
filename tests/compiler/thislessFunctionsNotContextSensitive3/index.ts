// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/thislessFunctionsNotContextSensitive3.ts`, Apache-2.0 License
  ...
  addProseMirrorPlugins() {
    return [
      Suggestion({
        ...
        //~^ ERROR: 'editor' is specified more than once, so this usage will be overwritten.
...
const childExtension = parentExtension.configure({
  ...
  //~^ ERROR: Object literal may only specify known properties, and 'child' does not exist in type 'Partial<{ parent: string; overwrite: string; }>'.
  overwrite: "child",
});
