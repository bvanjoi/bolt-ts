// From `github.com/microsoft/TypeScript/blob/v6.0.3/tests/cases/compiler/nestedExcessPropertyChecking.ts`, Apache-2.0 License
...
//~^ ERROR: Type '{ c: string; }' has no properties in common with type '{ a: undefined | string; } & { b: undefined | string; }'.
...
//~^ ERROR: Type 'C2' has no properties in common with type 'A2 & B2'.

enum E { A = "A" }

let x: { nope?: any } = E.A;  // Error
//~^ ERROR: Type 'E.A' has no properties in common with type '{ nope: any; }'.
...
//~^ ERROR: Type 'false' has no properties in common with type 'OverridesInput'.
